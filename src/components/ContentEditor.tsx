...
export default function ContentEditor({ content, onSave, onCancel, isOpen, title }: ContentEditorProps) {
  ...
  const renderField = (label: string, path: string, type: string = 'text', options?: string[]) => {
    const value = path.split('.').reduce((obj, key) => obj?.[key], editedContent) || '';
  ...
  const renderArrayField = (label: string, path: string, itemTemplate: any) => {
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1 bg-purple text-white rounded-lg hover:bg-phlox transition-colors text-sm"
          >
